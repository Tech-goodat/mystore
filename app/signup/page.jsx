...
const SignUp = () => {
    ...
    const handleSignUp = (e) => {
        ...
        .then((data) => {
            ...
            setToken(data.token) // Update state with the token
            setShowOTPModal(true) // Show OTP modal after successful signup
        })
        .catch(error => {
            console.error('Error: Cannot sign you in', error)
        })
    }
    ...
    return (
        <div className='flex flex-col lg:grid text-[#131524] lg:grid-cols-3 md:w-full h-screen items-center'>
            ...
            <div className='flex items-center justify-center col-span-2 flex-col h-full'>
                ...
                <form onSubmit={handleSignUp} className='w-full p-1 md:w-[400px] md:mt-7'>
                ...
                <div className='mt-4'>
                    <p className='text-[10px]'>Already have an account? <Link className='text-[#FA7275]' href='/'>Sign in</Link></p>
                </div>
                {showOTPModal && <OTPModal />}
            </div>

            {/* Render OTP Modal when showOTPModal is true */}
